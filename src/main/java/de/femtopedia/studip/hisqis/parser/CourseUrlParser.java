...
public class CourseUrlParser {
    ...
    public List<String> parse() throws IllegalAccessException, IOException, OAuthException {
        ...
        Document doc = Jsoup.parse(client.get(HisqisAPI.BASE_URL + "rds?state=notenspiegelStudent&next=tree"
                + ".vm&nextdir=qispos/notenspiegel/student&menuid=notenspiegelStudent&breadcrumb=notenspiegel"
                + "&breadCrumbSource=menu&asi=" + client.getAsi()).readLine());
